...
- **Fail-open.** If the HIBP API errors or times out (2s connect + 2s read), the check is skipped and registration proceeds — this is deliberately a defense-in-depth layer, not a hard dependency for account creation, consistent with how `EmailVerificationService.sendVerification()` already treats a best-effort external call.
