...
Everything above is active scanning this project runs itself. This addendum covers passive, GitHub-native monitoring — visibility that exists independently of any workflow file in this repo, applying to both `apps/frontend` (npm) and `apps/backend` (Gradle/Maven), where the controls above are frontend-only.

Enabled on `ChloeDelphis/proust-club` (public repo) via the GitHub API (`PUT /repos/{owner}/{repo}/vulnerability-alerts`, `PUT /repos/{owner}/{repo}/automated-security-fixes`) and, for malware alerts, the repository Settings UI (no REST endpoint exists for that toggle as of this writing):

- **Dependabot alerts** — GitHub-native equivalent of `pnpm audit`'s "known vulnerability" check (CVE/GHSA), but computed from the dependency graph rather than a local lockfile scan, and covering every ecosystem GitHub can parse for this repo, not just npm.
- **Dependabot security updates** — automatic PRs bumping a vulnerable dependency to a patched version, once an alert fires. No auto-merge configured: every such PR still has to pass the same required, non-bypassable `gitleaks`/`backend`/`frontend` status checks (branch protection, `enforce_admins: true`) as any other PR before it can land on `master`. Review is *not* separately required by branch protection (`required_approving_review_count: 0`) — merging still relies on Chloé looking at the PR, not on a GitHub-enforced review gate.
- **Dependabot malware alerts** — GitHub-native equivalent of the `OSV MAL-*` check `pnpm check:supply-chain` already runs locally (see `docs/features/supply-chain-check.md`), across all ecosystems GitHub covers for malware advisories (npm included), not just the one this repo's own script targets.

**Backend (Gradle) coverage — confirmed incomplete at enablement time, pending verification:** immediately after enabling the above, the dependency graph (`GET /repos/{owner}/{repo}/dependency-graph/sbom`) listed 366 packages — the repo itself, 5 GitHub Actions, and 360 npm packages. **Zero Maven/Gradle packages.** GitHub's static manifest parsing does not resolve `apps/backend/build.gradle.kts`, consistent with most of its dependencies having no inline version (resolved via the `io.spring.dependency-management` plugin instead) — nothing for static parsing to read.

Per the plan agreed with Chloé: **Automatic Dependency Submission** (Settings → Code security → Dependency graph) was enabled instead of hand-writing a `gradle/actions/dependency-submission` workflow — GitHub's native mechanism, no workflow file to maintain, Gradle-supported since 2025. It runs a GitHub-managed job on pushes to `master` that touch a manifest, submitting the actually-resolved dependency snapshot (sidestepping the inline-version problem above). Nothing has pushed to `master` since it was enabled, so **backend coverage is not yet verified** — to be checked after this branch is merged (a manifest-adjacent push): re-query the SBOM endpoint for `maven`/`gradle` packages. If still empty after a `master` push, fall back to the dedicated `gradle/actions/dependency-submission` workflow originally considered.

Documented in more detail, alongside the existing frontend controls, in `docs/architecture/dependency-security-strategy.md`.
