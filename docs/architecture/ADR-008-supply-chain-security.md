...
Enabled on `proust-club/proust-club` (public repo) via the GitHub API (`PUT /repos/{owner}/{repo}/vulnerability-alerts`, `PUT /repos/{owner}/{repo}/automated-security-fixes`) and, for malware alerts, the repository Settings UI (no REST endpoint exists for that toggle as of this writing):
...
- **Dependabot security updates** — automatic PRs bumping a vulnerable dependency to a patched version, once an alert fires. No auto-merge configured: every such PR still goes through the same required, non-bypassable `frontend-ci`/`backend-ci` checks (and human review) as any other PR before it can land on `master`.
